 ...
 *   AC1 — test.heading in en/common.json is exactly "SthlmTech"
 ...
 *   AC4 — The rendered <h1> contains the text "SthlmTech" (using real en/common.json)
 */
...
// ---------------------------------------------------------------------------
// AC1 — test.heading value in en/common.json is exactly "SthlmTech"
// ---------------------------------------------------------------------------

describe("AC1 — test.heading value in en/common.json", () => {
  it('is exactly "SthlmTech"', () => {
    expect(enCommon.test.heading).toBe("SthlmTech");
  });
});
...
describe("AC3 — test.tsx renders heading via t('test.heading'), no hardcoded string", () => {
  ...
  it('does not contain a hardcoded "SthlmTech" string in JSX', () => {
    ...
    const codeLines = testSourceRaw
      ...
      .join("\n");
    expect(codeLines).not.toContain("SthlmTech");
  });
});

// ---------------------------------------------------------------------------
// AC4 — Rendered <h1> contains "SthlmTech" (using real en/common.json)
// ---------------------------------------------------------------------------

describe("AC4 — TestPage renders <h1> with text 'SthlmTech'", () => {
  it("renders a heading element containing 'SthlmTech'", async () => {
    renderWithProviders(<TestPage />);

    // The h1 is rendered synchronously — heading text does not depend on data loading
    const heading = await screen.findByRole("heading", { level: 1 });
    expect(heading).toHaveTextContent("SthlmTech");
  });
});
