...
export function extractToolResult(text: string): ToolResultPayload | null {
...
export function isInternalGuardrailMessage(text: string): boolean {
  return text.startsWith(INTERNAL_GUARDRAIL_PREFIX);
}

export function isInternalAutoStartMessage(text: string): boolean {
...
function compactToolValue(value: unknown, depth = 0): unknown {
  if (typeof value === "string") {
    return value.length <= MAX_TOOL_STRING_LENGTH ? value : `${value.slice(0, MAX_TOOL_STRING_LENGTH)}…`;
  }

  if (Array.isArray(value)) {
    const compactedItems = value.slice(0, MAX_TOOL_ARRAY_ITEMS).map((item) => compactToolValue(item, depth + 1));
...
export function buildToolResultMessage(result: Awaited<ReturnType<typeof executeAIToolCall>>): string {
  const basePayload = {
    type: "tool_result",
