...
export async function listResumeBranches(
  ...
  const visibleRows = rows.filter((row) => {
    ...
    // Revision branches are never hidden by the main-reachability filter —
    // they may intentionally share a HEAD commit with their source variant.
    if (row.branch_type === "revision") {
      return true;
    }
