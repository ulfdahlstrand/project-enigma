...
export type AutomaticRevisionAssignment = {
  assignmentId: string;
  clientName: string;
  role: string;
};
...
export function buildAutomaticBroadRevisionWorkItems(
  scope: "whole_resume" | "all_assignments",
  assignments: AutomaticRevisionAssignment[],
): RevisionWorkItemInput[] {
  const sectionItems: RevisionWorkItemInput[] = scope === "whole_resume"
    ? [
        {
          id: "section-title",
          title: "Review title",
          description: "Check the resume title for the requested revision goal.",
          section: "title",
          status: "pending",
        },
        {
          id: "section-consultant-title",
          title: "Review consultant title",
          description: "Check the consultant title for the requested revision goal.",
          section: "consultantTitle",
          status: "pending",
        },
        {
        ...
        },
        {
          ...
          description: "Check the summary text for the requested revision goal.",
          section: "summary",
          status: "pending",
        },
      ]
  ...
  const assignmentItems = assignments.map((assignment, index) => ({
    ...
    title: `Review assignment: ${assignment.clientName}`,
    description: `Check the assignment text for ${assignment.clientName} (${assignment.role}) for the requested revision goal.`,
    section: "assignment",
    assignmentId: assignment.assignmentId,
    status: "pending",
  }));

  return [...sectionItems, ...assignmentItems];
}

export async function replacePersistedRevisionWorkItems(
  db: Kysely<Database>,
  input: {
    conversationId: string;
    branchId: string;
    items: RevisionWorkItemInput[];
  },
) {
  const nextItems = applyRevisionToolCallToWorkItems([], {
    toolName: "set_revision_work_items",
    input: {
      summary: "Autogenerated revision work items",
      items: input.items,
    },
  });
...
export async function replacePersistedAutomaticBroadRevisionWorkItems(
  db: Kysely<Database>,
  input: {
    conversationId: string;
    branchId: string;
