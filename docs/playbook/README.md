...
| Adding a frontend read-only route (list/detail view) | [`frontend-read-route.md`](./frontend-read-route.md) | Adding a new read-only page that fetches data from the backend via oRPC + TanStack Query, renders with MUI components, and translates all text via react-i18next |
| Adding a backend oRPC read procedure | [`backend-orpc-procedure.md`](./backend-orpc-procedure.md) | Adding a new `list*` or `get*` procedure: Zod schemas in `@cv-tool/contracts`, Kysely table types, oRPC handler with DI factory, router registration, and co-located Vitest unit test |
